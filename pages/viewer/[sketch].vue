...
const sketch = await queryContent('/sketches').findOne().then((sketches) => sketches.data.find((sketch) => sketch.title
  == useRoute().params.sketch));
...
function formatCode(code) {
  ...
  for (let i = 0; i < splittedCode.length - 1; i++) {
    formattedCode += `<span style="color: #666">${i}:</span>
  ${splittedCode[i]}\n`;
  }
...
<template>
  <div class="w-full flex justify-center">
    <main class="flex flex-col items-center min-h-screen max-w-screen-xl w-full">
      <NuxtLink to="/explorer" class="text-3xl mt-16 self-end">X</NuxtLink>
      ...
      <IconsArrowDownIcon class="mb-32" />
      <!-- Detail Panel -->
